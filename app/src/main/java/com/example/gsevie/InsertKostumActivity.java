    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        simpanKostum.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(namaKostum.getText().toString().length()==0 || jumlahKostum.getText().toString().length()==0 ||
                hargaKostum.getText().toString().length()==0 || deskripsiKostum.getText().toString().length()==0 || imagePath.isEmpty()) {
                    Toast.makeText(InsertKostumActivity.this, "Data Kostum tidak boleh kosong!", Toast.LENGTH_SHORT).show();
...
//                            (kategori.getSelectedItem().toString()));
                    ...
                    Call<GetKostum> kostumCall =mApiInterface.postKostum(body,reqid_kategori,reqnama_kostum,reqjumlah_kostum,
            ...
            public void onResponse(Call<GetKategori> call, Response<GetKategori> response) {
                if(response.body().getStatus().equals("success")){
                    ...
                    for(int i=0; i<kategoriItem.size();i++){
                        listSpinner.add(kategoriItem.get(i).getKategori());
                    }
